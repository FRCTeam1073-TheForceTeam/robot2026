...
void DrivePath::Execute() {
  ...
  if (trajectory.has_value()) {
    ...
    if(currentSample.has_value()) {
      ...
      xVelocity = xController.Calculate(robotPose.X().value(), traj_sample.x.value()) * 1_mps + (trajectory_speeds.vx * 1.0);
      yVelocity = yController.Calculate(robotPose.Y().value(), traj_sample.y.value()) * 1_mps + (trajectory_speeds.vy * 1.0); 
      thetaVelocity = thetaController.Calculate(robotPose.Rotation().Radians().value(), traj_sample.heading.value()) * 1_rad_per_s + (trajectory_speeds.omega * 1.0);
