...
_commandVelocityVoltage(units::angular_velocity::turns_per_second_t(0.0)) {
  ...
  if (!_hardwareConfigured) {
    std::cerr << "Kicker: Hardware Failed To Configure!" << std::endl;
  }
...
configs::TalonFXConfiguration configs{};
    ...
    if (!status.IsOK()) {
        std::cerr << "Kicker: Configuration went wrong" << std::endl;
    }
    ...
    if (!status.IsOK()) {
        std::cerr << "Kicker: Neutral brake went wrong" << std::endl;
    }
