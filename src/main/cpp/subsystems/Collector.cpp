...
void Collector::SetCommand(Command cmd) {
...
void Collector::Periodic() {
  ...
  _feedback.velocity = _velocitySig.GetValue() / (TurnsPerMeter * GearRatio); // Convert from hardare units to subsystem units.
  ...
  if (std::holds_alternative<units::velocity::meters_per_second_t>(_command)) {
      ...
      // Send to hardware:
      _motor.SetControl(_commandVelocityVoltage.WithVelocity(motorVel));
